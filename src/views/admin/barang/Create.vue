<template>
  <div class="container">
    <h1 class="text-4xl mb-6">Tambah Barang</h1>
    ...
    <Card>
      <template #content>
        <form action="#" @submit.prevent="storeBarang">
          <div class="grid grid-cols-2 gap-4">
           ...
           <div class="flex justify-end">
            <Button rounded severity="contrast" type="submit" class="px-6" label="Simpan" icon="pi pi-save" :loading="loading" />
          </div>
        </form>
      </template>
    </Card>
...
export default {
  ...
  mounted(){
    this.$store.dispatch('setBreadcrumbs', [
      ...
      {label: 'Tambah', route: '/admin/barang/create'}
    ]);
  },
  methods:{
    storeBarang(){
      this.loading = true;
      this.errors = {};
      axios.post('http://localhost:5000/api/barang/store', this.barang)
